...
function ArticleDetail({ task, post, related, comments }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="bg-[#F1E2D1]">
      <header className="border-b border-[rgba(84,26,26,.18)] bg-[#F1E2D1]">
        <div className="mx-auto max-w-[1120px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-[rgba(84,26,26,.18)] pt-4 text-[11px] font-black uppercase tracking-[0.16em]">
            <span className="text-[#810B38]">{categoryOf(post, 'News')}</span>
            {published ? <time>{published}</time> : null}
          </div>
          <h1 className="mt-6 max-w-6xl text-5xl font-black leading-[0.94] tracking-[-0.055em] text-[#541A1A] sm:text-6xl lg:text-[5.2rem]">{post.title}</h1>
          {summaryText(post) ? <p className="mt-6 max-w-4xl text-xl font-semibold leading-8 text-[#541A1A] sm:text-2xl">{summaryText(post)}</p> : null}
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto mt-8 max-w-[1120px] overflow-hidden rounded-[2.2rem] border border-[rgba(84,26,26,.14)] bg-[#F1E2D1] shadow-[0_22px_50px_rgba(84,26,26,.06)]">
          <img src={images[0]} alt="" className="max-h-[760px] w-full object-cover" />
          <figcaption className="border-t border-[rgba(84,26,26,.12)] px-4 py-3 text-xs italic text-[#541A1A] sm:px-6">Featured image for {post.title}</figcaption>
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1120px] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,740px)_340px] lg:px-8 lg:py-16">
        <article className="min-w-0 rounded-[2.2rem] border border-[rgba(84,26,26,.14)] bg-[#F1E2D1] p-6 shadow-[0_22px_50px_rgba(84,26,26,.06)] sm:p-8">
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[1.9rem] border border-[rgba(84,26,26,.14)] bg-[#F1E2D1] p-6 shadow-[0_16px_32px_rgba(84,26,26,.05)]">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[#810B38]">About this post</p>
          <div className="mt-4 grid gap-3 text-sm font-bold text-[#541A1A]">
      ...
      {related.length ? (
        <div className="rounded-[1.9rem] border border-[rgba(84,26,26,.14)] bg-[#F1E2D1] p-6 shadow-[0_16px_32px_rgba(84,26,26,.05)]">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-lg font-black tracking-[-0.04em] text-[#541A1A]">More like this</h2>
            <Link href={taskConfig?.route || '/'} className="text-xs font-black uppercase tracking-[0.16em] text-[#810B38]">View all</Link>
          </div>
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 rounded-[1.4rem] border border-[rgba(84,26,26,.1)] p-3 transition hover:border-[#810B38] hover:text-[#810B38]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-[1rem] object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-[1rem] bg-[#541A1A] text-[#F1E2D1]"><FileText className="h-6 w-6" /></div>}
      <div className="min-w-0">
        <h3 className="line-clamp-3 text-sm font-black leading-tight tracking-[-0.03em]">{post.title}</h3>
        <p className="mt-2 line-clamp-2 text-xs leading-5 text-[#541A1A]">{summaryText(post)}</p>
      </div>
    </Link>
  )
}
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 rounded-[1.9rem] border border-[rgba(84,26,26,.14)] bg-[#DCC3AA] p-5">
      <div className="flex items-center gap-2 text-lg font-black text-[#541A1A]"><MessageCircle className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="rounded-[1.3rem] border border-[rgba(84,26,26,.1)] bg-[#F1E2D1] p-4 shadow-sm">
            <p className="text-sm font-black text-[#541A1A]">{comment.name}</p>
            <p className="mt-2 text-sm leading-7 text-[#541A1A]">{comment.comment}</p>
          </div>
        ))}
        {!comments.length ? <p className="text-sm text-[#541A1A]">No comments yet for {slug}.</p> : null}
      </div>
    </section>
  )
}
